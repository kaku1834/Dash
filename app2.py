...
# st.write(raw.head(3))
# st.write(stock.head(3))
# st.write(dateInfo.head(3))
...
raw_salesD = raw_filtered.group_by('Date').agg(pl.col('Sales').sum(), pl.col('SalesPred').sum()).sort('Date')

# Merge raw sales data with dateInfo
raw_salesD_dateInfo = dateInfo.join(raw_salesD, on = 'Date', how = 'right')
# st.write(raw_salesD_dateInfo.head())
# Calculate Weekly Data
stock = stock.sort('Date')
raw_stockW = stock.group_by_dynamic('Date', 
                       every = '1w',
                       start_by = 'monday',
                       closed = 'right',
                       label = 'right',
                       offset = '6d').agg(pl.all().sum())

# Fixed Data
raw_salesD_dateInfo_1 = raw_salesD_dateInfo.select(['Date', 'Temperature', 'Event'])
# For calculate weekly data
raw_salesD_dateInfo_2 = raw_salesD_dateInfo.select(['Date', 'Sales', 'SalesPred', 'Customers', 'EventNum'])
...
df_disp_pl = raw_salesW_dateInfo.join(raw_stockW, on='Date', how='left').with_columns([
    ...
    pl.col("SalesPred").fill_null(strategy="backward"),
...
max_event_num = int(df_disp_events['EventNum'].max())

df_disp_events = df_disp.dropna(subset=['EventNum'])
ax1[0].bar(df_disp_events['Date'], df_disp_events['EventNum'], color=color0, label='Holidays', width = 1.5)
ax1[0].set_ylabel('Holidays', color=color0)
ax1[0].set_yticks(list(range(0, max_event_num + 1)))
ax1[0].tick_params(axis='y', labelcolor=color0)
...
ax1[1].plot(df_pred['Date'], df_pred['SalesPred'], color=color2, label='SalesPred', alpha=0.5, linewidth=2)
ax1[1].bar(df_pred['Date'], df_pred['Sales'], color= 'orange', label='Sales', alpha=1)
ax1[1].set_ylabel('Sales', color=color1)
